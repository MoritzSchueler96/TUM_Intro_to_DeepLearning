...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        x_hat = x - x_mean
        x_hat = x_hat / np.sqrt(x_var + eps)
        out = gamma * x_hat + beta

        running_mean = momentum * running_mean + (1-momentum) * x_mean
        running_var = momentum * running_var + (1-momentum) * x_var

        cache = (out, x_hat, beta, gamma, x_mean, x_var, eps)
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    out, x_hat, beta, gamma, x_mean, x_var, eps = cache

    dbeta = np.sum(dout, axis=0)
    dgamma = np.sum(dout * x_hat, axis=0)
    dx_hat = dout * gamma
    dx = (N * dx_hat - np.sum(dx_hat, axis=0) - x_hat * np.sum(dx_hat * x_hat, axis=0)) / (N * np.sqrt(x_var + eps))
