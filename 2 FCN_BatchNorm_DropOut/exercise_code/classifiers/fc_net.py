...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        scores1, cache0 = affine_forward(X, W1, b1)
        scores1_relu, cache1 = relu_forward(scores1)

        scores2, cache2 = affine_forward(scores1_relu, W2, b2)
        scores = scores2
        ...
        nll, dnll = softmax_loss(scores, y)

        reg_loss = reg * 0.5 * (np.sum(W1 * W1) + np.sum(W2 * W2))

        loss = nll + reg_loss

        dscores2, dw2, db2 = affine_backward(dnll, cache2)
        grads['W2'] = dw2 + reg * W2
        grads['b2'] = db2

        dscores1 = relu_backward(dscores2, cache1)

        dscores0, dw1, db1 = affine_backward(dscores1, cache0)
        grads['W1'] = dw1 + reg * W1
        grads['b1'] = db1
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        for index in range(len(layers) - 1):
            self.params['W' + str(index + 1)] = weight_scale * np.random.randn(layers[index], layers[index + 1])
            self.params['b' + str(index + 1)] = np.zeros(layers[index + 1])
    ...
    def loss(self, X, y=None):
        ...
        for index in range(len(layers) - 1):
            if index == 0:  # First layer
                scoreX, cache = affine_relu_forward(X, self.params['W' + str(index + 1)],
                                                    self.params['b' + str(index + 1)])
            elif index == len(layers) - 2:  # Last layer
                scores, cache = affine_forward(scoreX, self.params['W' + str(index + 1)],
                                               self.params['b' + str(index + 1)])
            else:  # other layers
                scoreX, cache = affine_relu_forward(scoreX, self.params['W' + str(index + 1)],
                                                    self.params['b' + str(index + 1)])
        ...
        for index in reversed(range(len(layers) - 1)):  # reversed because backprop
            if index == len(layers) - 2:  # Last layer
                dlast, dW, db = affine_backward(dscores, caches[index])
            else:
                dlast, dW, db = affine_relu_backward(dlast, caches[index])
            # compute loss
            loss += 0.5 * self.reg * np.sum(self.params['W' + str(index + 1)] * self.params['W' + str(index + 1)])
            # store gradients
            grads['W' + str(index + 1)] = dW + self.reg * self.params['W' + str(index + 1)]
            grads['b' + str(index + 1)] = db
